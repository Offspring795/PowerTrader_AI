...
class CryptoAPITrading:
    def __init__(self):
        ...
        # Same hard DCA triggers as original
        ...
        # Initialize cost basis & DCA levels (same logic, now via Kraken orders)
    ...
    def _fmt_price(price: float) -> str:
        """
        Same dynamic decimal formatting as original.
        """
        try:
            p = float(price)
        except Exception:
    ...
    def _read_long_price_levels(symbol: str) -> list:
        """
        Same low_bound_prices.html reader as original N1..N7 blue lines.
        """
        sym = str(symbol).upper().strip()
        folder = base_paths.get(sym, main_dir if sym == "BTC" else os.path.join(main_dir, sym))
    ...
    # -----------------------------
    # DCA WINDOW SEEDING (unchanged logic)
    # -----------------------------

    def _seed_dca_window_from_history(self) -> None:
        """
        Same logic as original, using local TRADE_HISTORY_PATH.
        """
        now_ts = time.time()
    ...
    # -----------------------------
    # KRAKEN ACCOUNT / HOLDINGS / ORDERS
    # -----------------------------
    ...
    def get_account(self) -> Any:
        """
        Returns an object with at least a 'buying_power' key, matching original shape.
        """
    ...
    def get_orders(self, symbol: str) -> Any:
        """
        Provides original-like order shape using Kraken ClosedOrders.
        Returns:
        {
          "results": [
            {
              ...
              "created_at": opentm,
              "executions": [
                {"quantity": vol_exec, "effective_price": avg_price, "fee": fee}
              ],
    ...
    # -----------------------------
    # COST BASIS / DCA LEVELS (original logic, now via Kraken)
    # -----------------------------
    ...
    def initialize_dca_levels(self):
        """
        Same DCA stage initialization logic as original, now using Kraken get_orders.
        """
        holdings = self.get_holdings()
        if not holdings or "results" not in holdings:
    ...
    def place_buy_order(
        ...
        """
        Market buy on Kraken for a USD amount.
        Preserves original behavior of logging an estimated fill at current_price.
        """
        ...
        volume = amount_in_usd / current_price
        ...
        res = self._kraken_private("/0/private/AddOrder", data)
        if not isinstance(res, dict):
            return None

        txids = res.get("txid") or res.get("txids") or []
    ...
    def place_sell_order(
        ...
        """
        Market sell on Kraken for a base-asset quantity.
        Logs the trade similar to original.
        """
        pair = self._kraken_pair_for_full_symbol(symbol)
        if not pair:
            return None
    ...
    # -----------------------------
    # ORIGINAL manage_trades (unchanged logic, now using Kraken methods)
    # -----------------------------
    ...
    def manage_trades(self):
        ...
        for holding in holdings.get("results", []):
            ...
            triggered_levels = triggered_levels_count

            current_stage = triggered_levels_count

            hard_next = self.dca_levels[current_stage] if current_stage < len(self.dca_levels) else self.dca_levels[-1]
            ...
            if avg_cost_basis > 0:
                ...
                if current_stage < 4:
                    neural_level_needed_disp = current_stage + 4
                    neural_levels = self._read_long_price_levels(symbol)
            ...
            print(
                f"\nSymbol: {symbol}"
                f"  |  DCA: {color}{dca_line_pct:+.2f}%{Style.RESET_ALL} @ {self._fmt_price(current_buy_price)} "
                f"(Line: {dca_line_price_disp} {dca_line_source} | Next: {next_dca_display})"
